...
describe('Tracked vs Untracked', () => {
	...
	bench(
		...
		() => {
			...
			const c = atom(3);
			const sum = computed(() =>
				untracked(() => a.value + b.value + c.value),
			);
			const _ = sum.value;
		...
		() => {
			...
			const result = computed(
				() =>
					tracked1.value +
					tracked2.value +
					untracked(() => untracked1.value + untracked2.value),
			);
...
describe('Untracked Performance', () => {
		...
		() => {
			...
			const sum = computed(
				() =>
					...
					untracked(() =>
						untrackedAtoms.reduce((acc, a) => acc + a.value, 0),
					),
			);
	...
	bench(
		...
		() => {
			const atoms = Array.from({ length: 10 }, () => atom(0));
			const sum = computed(() =>
				untracked(() => atoms.reduce((acc, a) => acc + a.value, 0)),
			);
			const _ = sum.value;
