...
describe('Dependency Chain Patterns', () => {
	...
	bench(
		...
		() => {
			...
			const level1 = Array.from({ length: 10 }, (_, i) =>
				computed(() => a.value * (i + 1)),
			);

			// Level 2: 10 computeds depend on level1
			const level2 = Array.from({ length: 10 }, (_, i) =>
				computed(() => level1[i].value * 2),
			);

			// Final: 1 computed depends on all level2
			const final = computed(() =>
				level2.reduce((sum, c) => sum + c.value, 0),
			);
	...
	bench(
		...
		() => {
			...
			let currentLevel: ReturnType<typeof computed<number>>[] = base.map((a) => computed(() => a.value));
