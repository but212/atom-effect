...
- **Small Vector Optimization (SVO)**: To minimize heap allocations and garbage collection (GC) pressure, the engine uses inline slots (`_s0` through `_s3`) for the most common dependency and subscriber links before falling back to dynamic arrays.
...
- **`DepSlotBuffer`**: A high-speed buffer for dependency tracking that features:
  - **Size Duality**: Separates physical capacity from logical size for rapid iteration.
