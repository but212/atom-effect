...
describe('DepSlotBuffer: Claiming (Cache Hits)', () => {
  ...
  bench('claimExisting 4 items (Inline hit)', () => {
    for (let i = 0; i < 4; i++) {
      buf4.claimExisting(deps4[i]!, i);
    }
  }, microBenchOptions);

  bench('claimExisting 16 items (Overflow hit)', () => {
    for (let i = 0; i < 16; i++) {
...
describe('DepSlotBuffer: Hashing (seal vs isDirty)', () => {
  ...
  bench('seal() + isDirtyFast() - 4 items', () => {
    buf4.seal();
    buf4.isDirtyFast();
  }, microBenchOptions);

  bench('seal() + isDirtyFast() - 16 items', () => {
    buf16.seal();
    buf16.isDirtyFast();
  }, microBenchOptions);
});
...
describe('DepSlotBuffer: Mega-node Threshold (Map Fallback)', () => {
  ...
  bench('claimExisting 64 items (Map fallback)', () => {
    for (let i = 0; i < 64; i++) {
...
describe('DepSlotBuffer: Truncation', () => {
  bench('truncateFrom(0) with 16 items', () => {
