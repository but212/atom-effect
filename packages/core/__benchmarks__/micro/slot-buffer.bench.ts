...
describe('SlotBuffer: Creation', () => {
  bench('new SlotBuffer()', () => {
    const buf = new SlotBuffer<number>();
    void buf;
  }, microBenchOptions);

  bench('new Array() baseline', () => {
...
describe('SlotBuffer: Addition (Inline <= 4)', () => {
  ...
  bench('push 4 items (Array baseline)', () => {
    const arr = [];
    arr.push(1);
    arr.push(2);
    arr.push(3);
...
describe('SlotBuffer: Addition (Overflow > 4)', () => {
  bench('add 16 items (SlotBuffer spill)', () => {
    const buf = new SlotBuffer<number>();
    for (let i = 0; i < 16; i++) {
      buf.add(i);
    }
  ...
  bench('push 16 items (Array baseline)', () => {
    const arr = [];
    for (let i = 0; i < 16; i++) {
      arr.push(i);
    }
...
describe('SlotBuffer: Churn (Gap Reuse)', () => {
  ...
  bench('remove 8 and add 8 (SlotBuffer O(1) reuse)', () => {
  ...
  bench('splice 8 and push 8 (Array baseline)', () => {
...
describe('SlotBuffer: Iteration', () => {
  ...
  bench('forEach 4 items (SlotBuffer)', () => {
    let sum = 0;
    inlineBuf.forEach(item => { sum += item; });
    void sum;
  }, microBenchOptions);

  bench('forEach 4 items (Array)', () => {
    let sum = 0;
    inlineArr.forEach(item => { sum += item; });
    void sum;
  }, microBenchOptions);

  bench('forEach 16 items (SlotBuffer)', () => {
    let sum = 0;
    overflowBuf.forEach(item => { sum += item; });
    void sum;
  }, microBenchOptions);

  bench('forEach 16 items (Array)', () => {
    let sum = 0;
    overflowArr.forEach(item => { sum += item; });
    void sum;
  }, microBenchOptions);
});

describe('SlotBuffer: Compaction', () => {
  bench('compact 16 items with 8 gaps (SlotBuffer)', () => {
    const buf = new SlotBuffer<number>();
    for (let i = 0; i < 16; i++) buf.add(i);
  ...
  bench('filter nulls (Array baseline equivalent)', () => {
    let arr: (number | null)[] = [];
    for (let i = 0; i < 16; i++) arr.push(i);
