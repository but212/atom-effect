...
describe('Efficiency: Batching vs Manual Propagation', () => {
  ...
  bench(`[Batch] form reset (20 fields, x${REPEATS})`, () => {
  ...
  bench(`[Manual] form reset (20 fields, x${REPEATS})`, () => {
    for (let j = 0; j < REPEATS; j++) {
      const nextVal = formFields[0]!.value === '' ? 'initial' : '';
      for (let i = 0; i < 20; i++) formFields[i]!.value = nextVal;
    }
  ...
  let _syncSink = 0;
  effect(() => { _syncSink = syncHeavy.value; }, benchEffectOptions);
  ...
  bench(
    ...
    () => {
      const createComponent = (id: number) => {
        ...
        const stop = effect(() => { keep(derived.value); }, benchEffectOptions);
        return { state, stop };
      };
