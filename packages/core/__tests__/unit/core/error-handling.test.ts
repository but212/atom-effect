...
import { sleep, tick, waitForScheduler } from '../../utils/test-helpers';
...
describe('Core - Error Handling and Propagation', () => {
  describe('Sync Error Propagation', () => {
    ...
    it('accumulates errors from multiple sources', () => {
      const x = computed(() => { throw new Error('X fail'); }, { defaultValue: 0 });
      const y = computed(() => { throw new Error('Y fail'); }, { defaultValue: 0 });
