...
export const ERROR_MESSAGES = {
  ...
  BLOCKED_EVENT_HANDLER: (name: string) => `Blocked setting dangerous event handler attribute "${name}".`,
  BLOCKED_PROTOCOL: (name: string) => `Blocked dangerous protocol in "${name}" attribute.`,
  BLOCKED_DANGEROUS_PROP: (name: string) => `Blocked setting dangerous property "${name}". Use html binding for sanitized HTML.`,
  INVALID_INPUT_ELEMENT: (tagName: string) => `Val binding used on non-input element <${tagName}>.`,
  DUPLICATE_KEY: (key: string | number, index: number) => `Duplicate key "${key}" at index ${index}.`,
} as const;
