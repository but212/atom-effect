...
describe('Bindings: One-way & Baselines', () => {
  test('one-way bindings and baselines comparison', async ({ bench }) => {
    await bench.compare(
      bench(
...
describe('Bindings: Two-way & Unified', () => {
  ...
  test('two-way and unified bindings comparison', async ({ bench }) => {
    await bench.compare(
      bench(
        'atom → DOM: input value (100 inputs x 100 updates)',
        withContainer(($container) => {
          const source = $.atom('initial');
          for (let i = 0; i < 100; i++) {
      ...
      bench(
        'DOM → atom: input value (trigger 100 events)',
        withContainer(($container) => {
          const source = $.atom('initial');
      ...
      bench(
        'textarea value (100 textareas x 100 updates)',
        withContainer(($container) => {
          const source = $.atom('initial');
          for (let i = 0; i < 100; i++) {
...
describe('Bindings: Form (atomForm)', () => {
  ...
  test('atomForm comparison', async ({ bench }) => {
    await bench.compare(
      bench(
      ...
      bench(
        ...
        withContainer(($container) => {
          ...
          for (let i = 0; i < 10; i++) {
            $(createFormHtml())
              .appendTo($container)
              .atomForm(profile, { validation: validationRules });
          }
        })
      ),
      microBenchOptions
