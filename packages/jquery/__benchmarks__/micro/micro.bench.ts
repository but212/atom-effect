...
describe('Bindings: One-way Propagation', () => {
  bench('create 100 text bindings', () => {
  ...
  bench('update text (100 elements x 50 updates)', () => {
    const $c = createContainer();
  ...
  bench('update html (100 elements x 20 updates)', () => {
    const $c = createContainer();
  ...
  bench('toggle class (100 elements x 100 toggles)', () => {
    const $c = createContainer();
  ...
  bench('composite binding (text+class+css+show) creation x 100', () => {
    const $c = createContainer();
    const text = $.atom('hello');
    const isActive = $.atom(true);
    const width = $.atom(100);
    for (let i = 0; i < 100; i++) {
      $('<div></div>').appendTo($c).atomBind({
        text,
...
describe('Bindings: Two-way (Input/Checked)', () => {
  bench('atom → DOM: input val (100 inputs x 100 updates)', () => {
    const $c = createContainer();
    const source = $.atom('initial');
  ...
  bench('DOM → atom: input val (trigger 100 events)', () => {
    const $c = createContainer();
    const source = $.atom('initial');
  ...
  bench('checkbox toggle (100 elements x 100 toggles)', () => {
    const $c = createContainer();
...
describe('List Rendering: atomList', () => {
  ...
  const makeItems = (count: number) => Array.from({ length: count }, (_, i) => ({ id: i + 1, text: `Item ${i + 1}` }));

  bench('initial render: 1000 items', () => {
    const $c = createContainer();
  ...
  bench('reconciliation: append 10 items to 100', () => {
    const $c = createContainer();
    const base = makeItems(100);
    const items = $.atom(base);
    $c.atomList(items, listOptions);
    items.value = [...base, ...Array.from({ length: 10 }, (_, i) => ({ id: 101 + i, text: `New ${101 + i}` }))];
    cleanupContainer($c);
  }, microBenchOptions);

  bench('reconciliation: full shuffle 100 items', () => {
    const $c = createContainer();
    const base = makeItems(100);
    const items = $.atom(base);
    $c.atomList(items, listOptions);
  ...
  bench('render 100 items with bind callback', () => {
    const $c = createContainer();
...
describe('Sanitization: sanitizeHtml', () => {
  ...
  bench('clean large (50+ nodes)', () => {
    sanitizeHtml(HTML_CLEAN_LARGE);
  }, microBenchOptions);

  bench('mixed dangerous attributes removal', () => {
    sanitizeHtml(HTML_MIXED_ATTRS);
  }, microBenchOptions);

  bench('batch throughput (100 × mixed profile)', () => {
