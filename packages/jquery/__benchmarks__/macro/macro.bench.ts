...
describe('Macro: Todo App Scenarios', () => {
  bench('full workflow: add(20) → toggle(10) → filter(active) → delete(5) → all', () => {
  ...
  bench('stats auto-update: 100 items with rate (toFixed)', () => {
    const $c = createContainer();
    const todos = $.atom<Todo[]>([]);
    ...
    $('<span class="rate"></span>').appendTo($c).atomText(rate, (v) => `${v.toFixed(1)}%`);

    todos.value = Array.from({ length: 100 }, (_, i) => ({ ...createTodo(`I${i}`), completed: i % 2 === 0 }));
    cleanupContainer($c);
  }, macroBenchOptions);
});
...
describe('Macro: Dashboard & Reactive Topology', () => {
  bench('20 widgets batch update (50 rounds)', () => {
    const $c = createContainer();
  ...
  bench('mount/unmount 20 components (10 cycles)', () => {
    const $c = createContainer();
    for (let cycle = 0; cycle < 10; cycle++) {
      ...
      for (const $s of slots) { $s.atomUnmount(); $s.remove(); }
    }
    cleanupContainer($c);
  }, macroBenchOptions);

  bench('deep propagation: 5-level chain → 20 DOM widgets (100 updates)', () => {
    const $c = createContainer();
    const source = $.atom(0);
  ...
  bench('fan-out: 1 atom → 20 computed → 20 DOM bindings', () => {
    const $c = createContainer();
    const source = $.atom(0);
    for (let i = 0; i < 20; i++) {
  ...
  bench('fan-in: 20 atoms → 1 computed → 1 DOM binding', () => {
    const $c = createContainer();
...
describe('Macro: atomForm O(1) Scaling', () => {
  ...
  bench('Update 1 field in 10-field form (x100)', () => {
    for (let i = 0; i < 100; i++) u10(i);
  }, macroBenchOptions);

  bench('Update 1 field in 100-field form (O(1) test, x100)', () => {
