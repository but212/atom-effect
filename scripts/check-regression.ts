...
async function checkRegression() {
  ...
  try {
    ...
    const currentResults: BenchmarkBaseline = JSON.parse(readFileSync(RESULTS_FILE, 'utf8'));
    ...
    for (const baselineResult of baseline.results) {
      const currentResult = currentResults.results.find((r) => r.name === baselineResult.name);
      ...
      const degradation =
        (baselineResult.opsPerSec - currentResult.opsPerSec) / baselineResult.opsPerSec;
    ...
    if (regressions.length > 0) {
      ...
      for (const reg of regressions) {
        ...
        console.error(`    Degradation: ${(reg.degradation * 100).toFixed(2)}%\n`);
      }
      console.error(`📈 Threshold: ${REGRESSION_THRESHOLD * 100}% degradation`);
      process.exit(1);
    }
