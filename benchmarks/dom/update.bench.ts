...
import { atom, batch, effect } from '../../src';
...
export async function runDOMUpdateBenchmark(): Promise<{ results: BenchmarkResult[] }> {
  ...
  const vanillaWithRead = results.find((r) =>
    r.name.includes('Vanilla JS: Update text (with read)')
  );
  ...
  if (vanillaWithRead && reactiveSync) {
    ...
    if (ratio > 1) {
      console.log(
        `   → Vanilla JS is ${((ratio - 1) * 100).toFixed(1)}% faster (expected due to no reactive overhead)`
      );
    } else {
